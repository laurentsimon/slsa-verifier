...
// VerifyProvenanceOptions is the top-level options for all `provenance` commands.
...
var _ Interface = (*VerifyProvenanceOptions)(nil)

// AddFlags implements Interface.
func (o *VerifyProvenanceOptions) AddFlags(cmd *cobra.Command) {
	/* Builder options */
...
func (o *VerifyVsaOptions) AddFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringSliceVarP(&o.VerifiedLevels, "verified-levels", "",
		[]string{}, "verified levels of the artifacts. SLSA_BUILD_LEVEL_1, SLSA_SOURCE_LEVEL_1, ...")
	cmd.Flags().BoolVar(&o.PrintVsa, "print-vsa", false,
		"[optional] print the verified VSA to stdout")
}

// VerifyNpmAttestationsOptions is the top-level options for the `verifyNpmPackage` provenance command.
type VerifyNpmAttestationsOptions struct {
...
var _ Interface = (*VerifyNpmAttestationsOptions)(nil)

// AddFlags implements Interface.
func (o *VerifyNpmAttestationsOptions) AddFlags(cmd *cobra.Command) {
	/* Builder options */
	cmd.Flags().Var(&o.BuildWorkflowInputs, "build-workflow-input",
		"[optional] a workflow input provided by a user at trigger time in the format 'key=value'. (Only for 'workflow_dispatch' events on GitHub Actions).")
